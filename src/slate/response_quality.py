...
def build_response_quality_prompt(
    *,
    ...
    section_lines = "\n".join(
        f'    "{key}": "{mode.value}",' if key == "mode" else f'    "{key}": ...,'
    ...
    return (
        ...
        f"from 0.0 to 1.0. Set `mode` exactly to `{mode.value}`. "
        "If evidence is missing, put that in `evidence`, `unknowns` and "
...
def missing_evidence_report(
    *,
    ...
    return ResponseQualityReport(
        ...
        evidence=[
            "No direct response-quality evidence was supplied; this report records the evidence gap."
        ],
        risks=[risk],
        counterarguments=[
