...
def test_required_lists_are_non_empty() -> None:
    with pytest.raises(ValidationError):
        ResponseQualityReport(
            mode=ResponseMode.EVIDENCE_BASED,
            facts=[],
...
def test_required_list_items_are_substantive() -> None:
    with pytest.raises(ValidationError):
        ResponseQualityReport(
            mode=ResponseMode.EVIDENCE_BASED,
            facts=["   "],
...
def test_missing_required_sections_lists_absent_keys() -> None:
    assert missing_required_sections({"facts": []}) == [
        "mode",
        *REQUIRED_SECTION_KEYS[2:],
    ]
...
def test_missing_evidence_report_acknowledges_gap() -> None:
    ...
    assert "evidence gap" in report.evidence[0]
